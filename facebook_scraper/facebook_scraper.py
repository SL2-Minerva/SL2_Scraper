...
class FacebookScraper:
    ...
    def get_posts(self, account: str, **kwargs) -> Iterator[Post]:
        ...
        iter_pages_fn = partial(
            iter_pages, account=account, request_fn=self.get, **kwargs)
    ...
    def get_reactors(self, post_id: int, **kwargs) -> Iterator[dict]:
        ...
        extractor = PostExtractor(
            response.html, kwargs, self.get, full_post_html=response.html)
        return extractor.extract_reactors(response)

    def get_photos(self, account: str, **kwargs) -> Iterator[Post]:
        iter_pages_fn = partial(
            iter_photos, account=account, request_fn=self.get, **kwargs)
        return self._generic_get_posts(extract_post, iter_pages_fn, **kwargs)
                ...
                if comments_area:
                    ...
                    try:
                        elem = utils.make_html_element(
                            elem.html.replace(
                                "</footer>", comments_area.html + "</footer>")
                        )
    ...
    def get_posts_by_search(self, word: str, **kwargs) -> Iterator[Post]:
        kwargs["scraper"] = self
        iter_pages_fn = partial(
            iter_search_pages, word=word, request_fn=self.get, **kwargs)
        return self._generic_get_posts(extract_post, iter_pages_fn, **kwargs)
    ...
    def get_friends(self, account, **kwargs) -> Iterator[Profile]:
        ...
        if not friend_url:
            friend_url = utils.urljoin(
                FB_MOBILE_BASE_URL, f'/{account}/friends/')
        ...
        while friend_url:
            ...
            for elem in elems:
                ...
                if tagline:
                    ...
                    tagline = ""
                # Profile Picture
                profile_picture = elem.find(
                    "i.profpic", first=True).attrs.get("style")
    ...
    def get_collection(self, more_url, limit=None, **kwargs) -> Iterator[Profile]:
        ...
        while more_url:
            ...
            if response.text.startswith("for (;;);"):
                ...
                # Strip 'for (;;);'
                data = json.loads(response.text[prefix_length:])
    ...
    def get_profile(self, account, **kwargs) -> Profile:
        ...
        if kwargs.get("allow_extra_requests", True):
            ...
            try:
                ...
                top_post = PostExtractor(
                    top_post, kwargs, self.get).extract_post()
                top_post.pop("source")
            ...
            try:
                result["Friend_count"] = utils.parse_int(
                    response.html.find(
                        "a[data-store*='friends']>div>div")[-1].text.split()[0]
                )
            except Exception as e:
            ...
            try:
                ...
                result["Following_count"] = utils.parse_int(
                    following_response.html.find(
                        "div[role='heading']", first=True).text
                )
            except Exception as e:
            ...
            if len(photo_links) == 1:
                ...
                if result["cover_photo_text"] is not None:
                    ...
                    extractor = PostExtractor(response.html, kwargs, self.get)
                    result["cover_photo"] = extractor.extract_photo_link_HQ(
                        response.html.html)
        ...
        for card in about.find("div[data-sigil='profile-card']"):
            ...
            if header in ["Work, Education"]:
                ...
                if len(bits) >= 3 and header == "Relationship":
                    result[header] = {"to": bits[0],
                                      "type": bits[1], "since": bits[2]}
                elif len(bits) == 1:
            ...
            if response.text.startswith("for (;;);"):
                ...
                for action in data['payload']['actions']:
                    if action['cmd'] == 'replace' and action['html']:
                        ...
                        elems = element.find(
                            '#page_suggestions_on_liking ~ div')
                    elif action['cmd'] == 'script':
                        more_url = re.search(
                ...
                more_url = re.search(
                    r'see_more_cards_id",href:"([^"]+)"', response.text)
                if more_url:
                    more_url = more_url.group(1)

            for elem in elems:
                header_elem = elem.find(
                    "div[data-nt='FB:TEXT4']:has(span)", first=True)
                if not header_elem:
                    continue
                ...
                if len(links) == 2:
                    user_url = utils.urljoin(
                        FB_BASE_URL, links[0].attrs["href"])
                else:
                    user_url = None
                text_elem = elem.find(
                ...
                yield {
                    ...
                    "post_url": utils.urljoin(
                        FB_BASE_URL, elem.find(
                            "a[href*='story']", first=True).attrs["href"]
                    ),
                }
    ...
    def get_page_info(self, page, **kwargs) -> Profile:
        ...
        try:
            ...
            result["name"] = resp.html.find(
                "title", first=True).text.replace(" - About", "")
        ...
        try:
            ...
            resp = self.get(url)
            result["id"] = re.search(
                r'pages/transparency/(\d+)', resp.html.html).group(1)
            result["name"] = resp.html.find(
                "title", first=True).text.replace(" - Home", "")
            desc = resp.html.find("meta[name='description']", first=True)
            ld_json = None
            try:
                ld_json = resp.html.find(
    ...
    def get_group_info(self, group, **kwargs) -> Profile:
        ...
        try:
            ...
            raise exceptions.UnexpectedResponse(
                "Unable to resolve view=info URL")
        logger.debug(f"Requesting page from: {url}")
        resp = self.get(url).html
        result = {}
        ...
        try:
            ...
            raise exceptions.UnexpectedResponse(
                "Unable to get one of name, type, or members")
        ...
        try:
            resp = self.get(url).html
            url = resp.find(
                "a[href*='listType=list_admin_moderator']", first=True)
            ...
            url = resp.find(
                "a[href*='listType=list_nonfriend_nonadmin']", first=True)
            if kwargs.get("members", True):
                if url:
                    ...
                    while url:
                        ...
                        resp = self.get(url).html
                        elems = resp.find(
                            "#root div.touchable a:not(.touchable)")
                        members.extend(
                            [{"name": e.text, "link": e.attrs["href"]} for e in elems])
                        more = re.search(
                            r'"m_more_item",href:"([^"]+)', resp.text)
                        if more:
                            url = more.group(1)
                        else:
                            url = None
                    result["other_members"] = [
                        m for m in members if m not in result["admins"]]
                else:
    ...
    def get_group_posts(self, group: Union[str, int], **kwargs) -> Iterator[Post]:
        ...
        iter_pages_fn = partial(
            iter_group_pages, group=group, request_fn=self.get, **kwargs)
    ...
    def check_locale(self, response):
        if self.have_checked_locale:
            return
        match = re.search(
            r'"IntlCurrentLocale",\[\],{code:"(\w{2}_\w{2})"}', response.text)
        if match:
            ...
            if DEBUG:
                for filename in os.listdir("."):
                    if filename.endswith(".html") and filename.replace(".html", "") in url:
                        logger.debug(
                            f"Replacing {url} content with {filename}")
                        with open(filename) as f:
                            response.html.html = f.read()
            response.html.html = response.html.html.replace(
                '<!--', '').replace('-->', '')
    ...
    def login(self, email: str, password: str):
        ...
        if "enter login code to continue" in response.text.lower():
            ...
            # Remember Browser
            response = self.submit_form(
                response, {"name_action_selected": "save_device"})
            if "review recent login" in response.text.lower():
                ...
                response = self.submit_form(
                    response, {"submit[This wasn't me]": None})
        ...
        if "login approval needed" in response.text.lower() or "checkpoint" in response.url:
            ...
            response = self.submit_form(
                response, {"submit[Continue]": "Continue"})
    ...
    def _generic_get_posts(
        ...
        **kwargs,
        ...
        if options is None:
            ...
            warnings.warn(
                "The options argument should be a dictionary.", stacklevel=3)
        ...
        # else, iterate over pages as usual
            ...
            counter = itertools.count(
                0) if page_limit is None else range(page_limit)
    ...
    def get_groups_by_search(self, word: str, **kwargs):
        group_search_url = utils.urljoin(
            FB_MOBILE_BASE_URL, f"search/groups/?q={word}")
